...
const Work = () => {
    ...
    const onSubmit = async (data) => {
        ...
        try {
            ...
            const errorMessage = error.response?.data?.message || 'Error: কাজ জমা দিতে পারেনি।';
    ...
    return (
        <div className="p-4 max-w-full mx-auto bg-white shadow-xl rounded-lg my-8">
            <h2 className="text-2xl font-bold text-gray-800 mb-6 border-b pb-2">জমা দিন: আপনার কাজ/প্রকল্প</h2>

            {/* Submission Status Message */}
            {submissionStatus && (
                <p
                    className={`mt-4 p-3 rounded-md text-center text-sm font-medium ${submissionStatus.includes('Success')
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col space-y-4 mt-6">
                ...
                <div>
                    ...
                    {errors.submitterName && (
                        <p className="text-red-500 text-xs mt-1">{errors.submitterName.message}</p>
                    )}
                </div>
                ...
                <div>
                    ...
                    />
                    {errors.workName && (
                        <p className="text-red-500 text-xs mt-1">{errors.workName.message}</p>
                    )}
                </div>
                ...
                <div>
                    ...
                    {errors.workCategory && (
                        <p className="text-red-500 text-xs mt-1">{errors.workCategory.message}</p>
                    )}
                </div>
                ...
                <div>
                    ...
                    />
                    {errors.workLink && (
                        <p className="text-red-500 text-xs mt-1">{errors.workLink.message}</p>
                    )}
                </div>

                {/* 6. Work Details / কাজের বিবরণ */}
                <div>
                    <label htmlFor="workDetails" className="block text-sm font-medium text-gray-700 mb-1">
                        **কাজের বিবরণ:**
                    </label>
                    <textarea
                        id="workDetails"
                    ...
                    ></textarea>
                    {errors.workDetails && (
                        <p className="text-red-500 text-xs mt-1">{errors.workDetails.message}</p>
                    )}
                </div>
                ...
                <button
                    ...
                    className={`w-full py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white transition duration-150 ease-in-out ${isSubmitting ? 'bg-gray-400 cursor-not-allowed' : 'bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500'
                        }`}
                >
