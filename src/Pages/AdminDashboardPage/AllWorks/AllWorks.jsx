...
const Toast = ({ message, type, onClose }) => {
  ...
  return (
    <div
      className={`fixed top-6 right-6 z-50 ${getStyles()} text-white px-6 py-4 rounded-xl shadow-2xl border-l-4 backdrop-blur-sm bg-opacity-95 transform animate-slide-in`}
    >
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md transform transition-all duration-300 scale-100">
        <div className="p-8 text-center">
          ...
          <p className="text-gray-600 mb-8 leading-relaxed">
            Are you sure you want to delete <strong>"{workName}"</strong>? This
...
const StatusBadge = ({ status }) => {
  const getStyles = () => {
    const baseStyles =
      "px-4 py-2 rounded-full text-sm font-semibold capitalize flex items-center space-x-2";
  ...
  const fetchWorks = useCallback(async () => {
    try {
      ...
      let userFriendlyError =
        "Unable to connect to the server. Please check if the server is running and the endpoint is correct.";
    ...
    return (
      <div className="text-center p-16 bg-gradient-to-br from-white to-gray-50 rounded-2xl mx-auto max-w-4xl mt-6 shadow-lg">
        ...
        <p className="text-gray-500 max-w-md mx-auto leading-relaxed">
          There are no submitted projects at the moment. As an admin, you'll see
          projects here once users start submitting their work.
        </p>
      </div>
    );
  }
